...
page / component canvas / node を対象にする tool は、`pageID` または `componentID` のどちらか一方で対象 HTML を指定する。`pageID` は `.ogp` の `chapters[].pages[]`、`componentID` は top-level `components[]` を指す。両方を同時に渡す、またはどちらも渡さない呼び出しは invalid である。
...
| `screenshot_node` | page または component canvas 内の `data-og-id` node を切り抜いた PNG に保存する | `screenshot node` |
...
page / component canvas / node を対象にする tool は `pageID` または `componentID` のどちらか一方を受け取る。node を対象にする tool は追加で `id` を `data-og-id` として扱う。
