...
## In-App Cache Synchronization

OpenGraphite app の中では、読み込み済み project / HTML / resource から作られたアプリ内 cache が、永続化前の編集状態を表します。ユーザー操作によって cache 上の値が変わった場合、その値を表示する Canvas、Layers、Inspector、Project 依存性ビューなどの複数経路は、保存完了やファイル再読み込みを待たずに同じ cache から同期して表示します。
