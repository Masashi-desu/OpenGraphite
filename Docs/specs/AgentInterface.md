...
OpenGraphite の AI 協業インターフェースは、リポジトリ上の HTML / CSS / `.ogp` を正本として扱う。AI、CLI、MCP は同じファイルを読み、同じ検証契約を通して、可能な限り `data-og-id` 単位の小さな編集操作として変更する。編集対象 HTML は必ず `.ogp` の `chapters[].pages[]` でユーザーが認知できる状態にしてから扱う。
...
`ogkiln page graph <project.ogp|current> --page-id <page-id> --json` と MCP の page graph resource は、`.ogp` の対象 page に含まれる `[data-og-id]` ノードを DOM 出現順に返す。
...
`.ogp` に登録されていない HTML は直接編集しない。既存 HTML は `project page add`、新規 HTML は `project page create` により既定 Chapter の `pages[]` に追加してから node operation の対象にする。
...
| node `id` | `data-og-id` |
| `get_editor_state` | `project inspect` + `page graph --page-id` |
...
任意プロジェクトや任意ページは tool の `projectPath` / `pageID` 引数で扱う。
...
`.ogp` project manifest も外部変更監視の対象にする。`ogkiln project page add`、`ogkiln project page create`、`ogkiln project page place` が page entry と canvas 配置を更新した場合、app は project manifest を再読み込みし、既存の選択 Chapter と選択ページを可能な限り維持したまま Canvas 上のページ配置を更新する。
