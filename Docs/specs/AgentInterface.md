...
OpenGraphite の AI 協業インターフェースは、リポジトリ上の HTML / CSS / `.ogp` を正本として扱う。AI、CLI、MCP は同じファイルを読み、同じ検証契約を通して、可能な限り `data-og-id` 単位の小さな編集操作として変更する。編集対象 HTML は必ず `.ogp` の `chapters[].pages[]` または top-level `components[]` でユーザーが認知できる状態にしてから扱う。
...
`ogkiln page graph <project.ogp|current> --page-id <page-id>|--component-id <component-id> --json` と MCP の graph resource は、`.ogp` の対象 page または component canvas に含まれる `[data-og-id]` ノードを DOM 出現順に返す。Components セグメントの HTML は `--component-id <component-id>` で同じ graph / node edit 経路を使う。
...
`data-og-id` は node edit operation の安定キーであり、DOM の位置や tag name は同一性として使わない。
...
ogkiln screenshot page SampleProject/OpenGraphiteSample.ogp --component-id design-system --output screenshots/design-system.png
ogkiln screenshot page SampleProject/OpenGraphiteSample.ogp --page-id docs --width 390 --height 900 --full-page --output screenshots/docs-mobile.png
ogkiln screenshot node SampleProject/OpenGraphiteSample.ogp --page-id docs --id doc-cli --output screenshots/doc-cli.png
ogkiln screenshot node SampleProject/OpenGraphiteSample.ogp --component-id design-system --id feature-card-master --output screenshots/feature-card.png
ogkiln build SampleProject/OpenGraphiteSample.ogp --output dist
ogkiln node query SampleProject/OpenGraphiteSample.ogp --page-id home --type button --text-contains Docs --json
ogkiln node query SampleProject/OpenGraphiteSample.ogp --component-id design-system --type frame --json
ogkiln node get SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --json
ogkiln node style set SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --var --og-gap --value 32px
ogkiln node style set SampleProject/OpenGraphiteSample.ogp --component-id design-system --id feature-card-master --var --og-padding --value 48px
ogkiln node style remove SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --var --og-gap
ogkiln node attr set SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --name data-og-role --value card
ogkiln node attr set SampleProject/OpenGraphiteSample.ogp --component-id design-system --id feature-card-master --name data-og-part --value root
ogkiln node attr remove SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --name data-og-role
ogkiln node text set SampleProject/OpenGraphiteSample.ogp --page-id home --id title --value 'OpenGraphite'
ogkiln node text set SampleProject/OpenGraphiteSample.ogp --component-id design-system --id feature-card-title --value 'Reusable card'
ogkiln node html insert SampleProject/OpenGraphiteSample.ogp --page-id home --id page --position prepend --html '<Header data-og-id="site-header" data-og-type="frame"></Header>'
ogkiln node html insert SampleProject/OpenGraphiteSample.ogp --component-id design-system --id component-page --position append --html '<FeatureCard data-og-id="feature-card-master" data-og-type="frame" data-og-component="feature-card" data-og-component-kind="master"></FeatureCard>'
ogkiln node html replace SampleProject/OpenGraphiteSample.ogp --page-id home --id hero --html '<Hero data-og-id="hero" data-og-type="frame"></Hero>'
ogkiln node delete SampleProject/OpenGraphiteSample.ogp --page-id home --id old-card
ogkiln node move SampleProject/OpenGraphiteSample.ogp --page-id home --id footer --target hero --position after
ogkiln node copy SampleProject/OpenGraphiteSample.ogp --page-id home --id card --target card-list --position append --id-prefix copy-
```

`.ogp` に登録されていない HTML は直接編集しない。既存 page HTML は `project page add`、新規 page HTML は `project page create` により既定 Chapter の `pages[]` に追加してから node operation の対象にする。component master を置く HTML は `project component add` または `project component create` により top-level `components[]` に追加し、`--component-id` で同じ node operation の対象にする。

write operation は次の制約を持つ。

- 対象 node は一意な `data-og-id` で解決できること。
- `data-og-id` が重複している場合は失敗すること。
- `--og-*` 以外の CSS 変数を `node style set` で更新しないこと。
- `data-og-selected` と `data-og-editing` は正本 HTML へ残さないこと。
- `data-og-id` は安定キーのため、通常の `node attr set` では変更しないこと。
...
- `node copy` は subtree 内の全 `data-og-id` に `--id-prefix` を付与し、既存 ID と衝突する場合は書き込まないこと。
...
`pageID` と `componentID` は同時指定できない。Pages HTML を編集する場合は `pageID`、Components セグメント内の component master HTML を編集する場合は `componentID` を指定する。
...
`.ogp` project manifest も外部変更監視の対象にする。`ogkiln project page add`、`ogkiln project page create`、`ogkiln project page place`、`ogkiln project component add`、`ogkiln project component create`、`ogkiln project component place`、`ogkiln project component remove` が entry と canvas 配置を更新した場合、app は project manifest を再読み込みし、既存の選択 Chapter と選択ページ / component canvas を可能な限り維持したまま Canvas 上の配置を更新する。
