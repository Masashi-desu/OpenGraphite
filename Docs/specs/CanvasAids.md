...
Guide の追加、移動、削除は最新 `.ogp` を再読込して対象 Chapter / Collection の `guides[]` だけへ atomic write し、「取り消す」`⌘Z` と「やり直す」`⇧⌘Z` の対象にします。Ruler からの追加、1回のドラッグ移動、Ruler 戻しまたはコンテキストメニューによる削除を、それぞれ1履歴単位として記録します。
