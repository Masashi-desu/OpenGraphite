...
- `text`: `--og-font-family`、`--og-font-size`、`--og-font-weight`、`--og-line-height`、`--og-letter-spacing`、`--og-text-align` を持つ block text。
...
- `--og-font-family-<locale>`（例: `--og-font-family-ja`, `--og-font-family-eng`, `--og-font-family-en-us`）
- `--og-font-size`
- `--og-font-weight`
- `--og-line-height`
- `--og-letter-spacing`
...
- Google Fonts など外部 Web font を使う場合、font family の選択値は `--og-font-family`、ページ標準は `--og-font-family-default`、locale 別標準は `--og-font-family-<locale>` に保持し、読み込みは通常の `<link rel="stylesheet" href="...">` として HTML `<head>` に残す。
- i18n の翻訳文は locale resource に置くが、locale 別の typography design value は page root の CSS 変数に置く。text node 側の `--og-font-family` はその要素だけの明示 override として扱う。
