...
/// 概要: HTML のicon provenanceと実体mask要素の標準CSS URLをProject Dependencies向けに集約できることを確認します。
...
struct OpenGraphiteIconCDNDependencyTests {
    ...
    func testAggregatesLucideCDNIcons() throws {
        // コンディション：CDN / inline / library source が混在した HTML を用意する（Given）
        let html = """
    ...
    func testLoadedProjectScanIncludesComponentMasters() throws {
        // コンディション：通常 page と component master を持つ一時 project を用意する（Given）
        ...
        try """
        :root {
          --icon-url: url('https://cdn.jsdelivr.net/npm/lucide-static@0.468.0/icons/panel-left-close.svg');
        }

        [data-og-internal-id="close-node"] > span {
        ...
        // 検証内容：project 全体から CDN icon 依存性を検出する（When）
        let dependencies = OpenGraphiteIconCDNDependencyScanner.dependencies(for: loadedProject)
        let dependency = try #require(dependencies.first)

        // 期待値：custom propertyで解決したpinned CDN iconがcomponent master依存性として表示できる（Then）
        #expect(dependencies.count == 1)
