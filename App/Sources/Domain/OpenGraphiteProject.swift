...
struct OpenGraphiteProject: Codable, Equatable {
    ...
    /// 論理名（日本語）: 内部ID正規化関数
    /// 処理概要: Chapter / Page / Collection / Component を先に確定し、注釈とガイドを後段で補完・重複解消します。
    ///
    ...
    func normalizedInternalIDs() -> OpenGraphiteProject {
        ...
        for chapterIndex in normalized.chapters.indices {
            for annotationIndex in normalized.chapters[chapterIndex].annotations.indices {
                let annotation = normalized.chapters[chapterIndex].annotations[annotationIndex]
                let annotationBase = Self.identityBase(
                    preferred: annotation.internalID,
                    semanticPrefix: "annotation",
                    seed: [
                        "annotation",
                        "pages",
                        "\(chapterIndex)",
                ...
                normalized.chapters[chapterIndex].annotations[annotationIndex].internalID = Self.uniqueIdentityID(
                    base: annotationBase,
                    used: &usedManifestIDs
                )
            }
        ...
        for collectionIndex in normalized.collections.indices {
            for annotationIndex in normalized.collections[collectionIndex].annotations.indices {
                let annotation = normalized.collections[collectionIndex].annotations[annotationIndex]
                let annotationBase = Self.identityBase(
                    preferred: annotation.internalID,
                    semanticPrefix: "annotation",
                    seed: [
                ...
                normalized.collections[collectionIndex].annotations[annotationIndex].internalID = Self.uniqueIdentityID(
                    base: annotationBase,
                    used: &usedManifestIDs
                )
            }
        ...
        for chapterIndex in normalized.chapters.indices {
            for guideIndex in normalized.chapters[chapterIndex].guides.indices {
                ...
                let guideBase = Self.identityBase(
                    preferred: guide.internalID,
                    semanticPrefix: "guide",
                    seed: [
                        "guide",
                        "pages",
                        "\(chapterIndex)",
                        "\(guideIndex)",
                        guide.orientation.rawValue,
                        "\(guide.position)"
                    ].joined(separator: "|")
                )
                normalized.chapters[chapterIndex].guides[guideIndex].internalID = Self.uniqueIdentityID(
                    base: guideBase,
                    used: &usedManifestIDs
                )
            }
        ...
        for collectionIndex in normalized.collections.indices {
            for guideIndex in normalized.collections[collectionIndex].guides.indices {
                let guide = normalized.collections[collectionIndex].guides[guideIndex]
                let guideBase = Self.identityBase(
                    preferred: guide.internalID,
                    semanticPrefix: "guide",
                    seed: [
                ...
                normalized.collections[collectionIndex].guides[guideIndex].internalID = Self.uniqueIdentityID(
                    base: guideBase,
                    used: &usedManifestIDs
                )
            }
...
enum OpenGraphiteProjectResourceSelection: Hashable, Equatable {
    ...
    var detail: String {
        ...
        case .iconCDN(_, let provider, let package, let version, _, _):
            let packageLabel = version.isEmpty ? package : "\(package)@\(version)"
            return "\(packageLabel) · \(provider)"
        case .i18nRuntime:
